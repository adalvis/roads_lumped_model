"""
Purpose: Road segment + ditchline grid creation based on figure conceptualizations
Date: 03/27/2018
...
for i in range(0,355): #loop through road length
    ...
    for j in range(0, 51): #loop through road width
        z[i*51 + j] = elev #update elevation based on x & y locations
        ...
        if j == 0 or j == 4:
            ...
            elev = -0.762
        elif j < road_peak and j > 3: #update latitudinal slopes based on location related to road_peak
            elev += up
        else:
            elev -= down
...
imshow_grid(mg, z, var_name = 'Elevation', 
            var_units = 'm',grid_units = ('m','m'), cmap = 'gist_earth')
plt.plot((3.825, 3.825), (0,79.875), 'r-')
plt.plot((5.175, 5.175), (0,79.875), 'r-')
plt.title('Road Surface Elevation', fontweight = 'bold')
