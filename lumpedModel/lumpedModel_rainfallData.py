...
df['stormNo'] = storm_index # len=78741
df['totalNo'] = df.stormNo.copy() # len=78741
df['groupedDepth'] = df.groupby('stormNo')['depth'].transform('sum') # len=78741
df.totalNo.fillna(method='ffill', inplace=True) # len=78741
df.fillna(0, inplace=True) # len=78741

test = df.groupby(['totalNo','depth'])['tips'].count() # len=425
...
day0 = t0
df_storm.set_index(pd.DatetimeIndex([day0+datetime.timedelta(hours=time) for time in df_storm.time]), inplace=True)
# #%%
